...
| 04G | Expansion: uncapped rerun + pre-drafting checks | DONE — core grid agrees (LABEL all layers, EVALUATE form, EXPRESS content+visual, CAPTION text-negativity all hold); 5 borderline category flips -> CHAT (omnibus_visual, RESPOND_content, STRUCTURAL_fw, CAPTION_visual, reg_EXPRESS_content); pure-carrier contrasts hold x3; 2 section-3 surprises -> CHAT (CAPTION content sign flips under mean-centring; EXPRESS instance-level content variant null vs centroid sig). Drafting blocked on these chat calls |
