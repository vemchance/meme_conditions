...
def main():
    ...
    emit("gloss_clauses.csv columns: {} ({} rows)".format(
        list(clauses.columns), len(clauses)))
    if "label_v2" not in clauses.columns:
        emit("DISCREPANCY: label_v2 absent - run scripts/03b_taxonomy_v2.py "
    ...
    emit("- Clause mass per family: {}".format(
        {f: int(clause_mass.get(f, 0)) for f in FAMILIES}))
    emit("- Contrast-eligible families (>= {} clauses): {}; descriptive "
    ...
    emit("OCR columns: {} ({} rows; join key 'label' per gate validation)".format(
    ...
    emit()
    emit("- Formats with >= 1 usage-family clause AND >= {} usable instances: "
         "{} (spec expectation ~1,607)".format(config.MIN_INSTANCES, len(F_all)))
    emit("- Excluded from pair classification (all clauses OTHER under v2): {}".format(
        len(unmapped_only)))
    emit("- Pair-classification population: {}".format(len(F)))
    n = len(F)
    ...
    emit("- Formats per family: " + ", ".join(
        "{} {}".format(fam, int(L[:, j].sum())) for j, fam in enumerate(FAMILIES)))
    emit()

    # ------------------------------------------------------------------
    emit("## 1. Instance-text preparation")
    emit()
    ...
    emit("- Token marks ({}): {}".format(len(token_marks), sorted(token_marks)))
    emit("- Phrase marks: {}; conditional (youtooz/cravi texts): {}".format(
    ...
    emit("- Capped instances (<= {} per format, seeded): {} from {} formats".format(
    ...
    if skipped_empty:
        emit("- NOTE: {} formats lost all instances to stripping+refilter; "
    ...
    drop = (~valid).sum()
    if drop:
        emit("- Dropping {} empty-profile formats from pair analyses.".format(drop))
    ...
    emit("- Profiles: {} formats; FW dim {}; POS-trigram vocab {}.".format(
        n, len(STOP), V))
    emit()
    S_fw = neg_jsd_matrix(P)
    S_pos = cosine_matrix(X)
    S_dict = {"fw_negjsd": S_fw, "pos_cosine": S_pos}
    ...
    # one permutation loop covers: headline, per-family one-vs-rest,
    # Searle grouping, EVALUATE->EXPRESS merge, MRQAP coefficient,
    # zero-shared-tags contrast
    ...
    for fid in F:
        ...
        type_sets.append(frozenset(t.strip().lower() for t in tp.split(";")
                                   if t.strip()))
    tag_vocab = {t: i for i, t in enumerate(sorted(set().union(*tags_sets) or {""}))}
    B = np.zeros((n, len(tag_vocab)), np.float32)
    for i, ts in enumerate(tags_sets):
        for t in ts:
            B[i, tag_vocab[t]] = 1.0
    inter = B @ B.T
    sizes = B.sum(1)
    ...
    type_vocab = {t: i for i, t in enumerate(sorted(set().union(*type_sets) or {""}))}
    ...
    log_n = np.log(np.array([per_format[fid] for fid in F], np.float64))
    ...
    zerotag = jacc_v == 0
    y_v = {meas: S[iu].astype(np.float64) for meas, S in S_dict.items()}

    Z = np.column_stack([np.ones(len(jacc_v)), jacc_v, stype_v, lsum_v]).astype(np.float64)
    ...
    def all_stats(Lc, Lsec, Lmec):
        ...
        for meas, S in S_dict.items():
            ...
            out[("mrqap_beta", meas)] = fwl_beta(m_v.astype(np.float64), meas)
            sv = y_v[meas]
            zm = zerotag & m_v
            zb = zerotag & ~m_v
    ...
    obs_stats = all_stats(L, L_se, L_me)
    null_stats = {k: np.empty(NP_, np.float32) for k in obs_stats}
    for t in range(NP_):
        idx = rng.permutation(n)
        st = all_stats(L[idx], L_se[idx], L_me[idx])
        for k, v in st.items():
            null_stats[k][t] = v
        if (t + 1) % 2000 == 0:
    ...
    # observed regression coefficients for the record
    ...
    for meas in S_dict:
        Xr = np.column_stack([np.ones(len(m_obs_v)), m_obs_v, jacc_v, stype_v, lsum_v])
        beta, *_ = np.linalg.lstsq(Xr, y_v[meas], rcond=None)
    ...
    for stratum in ("exploitable", "image macro", "catchphrase"):
        ...
        ns = int(sub.sum())
        if ns < 10:
            emit("- {}: only {} formats, skipped.".format(stratum, ns))
            continue
        sub_idx = np.where(sub)[0]
        Ls = L[sub_idx]
        Ss = {meas: S[np.ix_(sub_idx, sub_idx)] for meas, S in S_dict.items()}
        r = run_headline(Ss, Ls, ns, NP_, np.random.default_rng(config.RANDOM_SEED),
    ...
    emit()
    fmt_fw_u = np.zeros((n, len(STOP)), np.float64)
    fmt_tri_u = [None] * n
    capped_by_fmt = dict(tuple(capped.groupby("format_id")))
    for fid in F:
        g = capped_by_fmt.get(fid)
        fw, tri, _ = profile_texts(g["text_raw"].tolist(), False)
        i = fid_index[fid]
        fmt_fw_u[i] = fw
        fmt_tri_u[i] = tri
    Pu = fmt_fw_u / np.maximum(fmt_fw_u.sum(1, keepdims=True), 1e-9)
    Xu = np.zeros((n, len(tri_index)), np.float32)
    for i, tri in enumerate(fmt_tri_u):
        for j, c in tri.items():
            Xu[i, j] = c
    Xu = Xu / np.maximum(Xu.sum(1, keepdims=True), 1e-9)
    Su = {"fw_negjsd": neg_jsd_matrix(Pu), "pos_cosine": cosine_matrix(Xu)}
